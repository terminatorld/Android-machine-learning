...
def parse_xml(path,p,act):#解析xml文件,获取权限和activity信息
    doc=parse(path)
            ...
            with open(p,'r',encoding='utf-8') as f:
                ...
                for line in lines:
                    if line.startswith('.method'):
                        ...
                        smali_parse_list.append('->'.join([lines[0].strip().rsplit(' ',1)[1],line.strip().rsplit(' ',1)[1]]))
                    elif 'invoke' in line:
                        try:#处理出现的InsexError，但出现原因不明
                            line=line.strip().rsplit(', ',1)[1][:-1]
        ...
        if line.startswith('.method'):
            ...
            while(lines[i+j].startswith('.end')!=True):
                if 'invoke' in lines[i+j]:
                    try:#处理出现的InsexError，但出现原因不明
                        a=['->'.join([lines[0].strip().rsplit(' ',1)[1],head.strip().rsplit(' ',1)[1]]),lines[i+j].strip().rsplit(', ',1)[1][:-1]]
                    except IndexError:
                        print('IndexError happened...')
                        continue
                    else:
...
if __name__=='__main__':
    ...
    path='C:\\Users\\yhm\\Desktop\\class\\classtodownload\\TOOLS'
#    path='F:\decompile\TESTCLASS'
    pat=re.compile(r'\'(.*?)\'')
    
    i=1
    for f in os.listdir(path):
        if os.path.isdir(os.path.join(path,f)):#判断是否为空文件夹
...
#            start_time = time.perf_counter()
            parse_xml(open_xml(os.path.join(path,f)),permissions,acts)
            get_smali_list(os.path.join(path,f),smali_name_list)
            parse_smali(acts,smali_name_list,smali_parse_list,api_call_list)
            w_in_txt(os.path.join(path,f),permissions,'p')
            w_in_txt(os.path.join(path,f),smali_parse_list,'s')
            w_in_txt(os.path.join(path,f),api_call_list,'a')
